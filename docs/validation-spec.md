...
- Validation reports now carry explicit benchmark metadata (`benchmark_type`, fixture family, methodology, and return-alignment contract) so benchmark assumptions are inspectable in code and exports rather than only implied by a benchmark name string.
- `src/stock_selection/backtest/benchmarks.py` now provides deterministic benchmark-family fixtures for the sample validation/reporting flows, making benchmark-family assumptions more concrete without pretending to be live point-in-time benchmark sourcing.
...
- `src/stock_selection/reporting.py` now provides deterministic reporting/export helpers for validation summaries, validation periods, explanation cards, benchmark fixtures, analysis-bundle manifests, and validation-report CSV bundles so the new diagnostics are inspectable without ad hoc serialization.
- Pipeline-backed sample CLI exports now exist for explanation cards, validation reports, benchmark fixtures, and a combined analysis bundle, while the demo ranking command remains explicitly demo-only.
- Underfilled periods now remain partially in cash instead of reweighting the available names to 100%; period results expose period index, requested-versus-selected counts, fill ratio, underfilled status, invested weight, cash weight, buy/sell turnover, next rebalance anchor, inferred holding-period days, benchmark-relative gap in bps, and cumulative portfolio/benchmark/excess returns explicitly.
...
- Benchmark-family fixtures make sample workflows more concrete, but they are still deterministic fixtures rather than live or point-in-time-safe benchmark sourcing.
- Benchmark series construction remains an external assumption for non-fixture use cases; the harness now requires more explicit benchmark metadata, but it still does not infer or verify benchmark constituents or methodology from raw market data on its own.
