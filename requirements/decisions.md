...
- FMP corporate-actions retrieval uses `historical-price-full/stock_dividend/{ticker}` and `historical-price-full/stock_split/{ticker}`; ownership/short-interest retrieval uses `key-metrics-ttm/{ticker}` and `short-interest/{ticker}` with explicit `None` for missing fields and explicit unsupported errors when endpoints are unreachable.
