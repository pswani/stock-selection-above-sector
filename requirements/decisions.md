...
- Benchmark-family fixtures now live in `src/stock_selection/backtest/benchmarks.py` for deterministic sample validation flows; they make sample benchmark methodology more concrete without pretending to be live market-sourced benchmarks.
...
- Explanation cards now also expose score gap to the top rank in addition to gap-to-next-rank, pillar-count coverage, minimum-pillar status, and penalty counts so ranking explanations connect more directly to missing-data and penalty state.
- Validation summaries and periods now expose fixture-family context, period start/end anchors, benchmark outperformance counts, and cumulative benchmark-relative progression so benchmark interpretation is easier outside Python objects.
- Reporting/CLI now support pipeline-backed benchmark-fixture exports plus a richer analysis bundle export that writes ranking, explanation, validation, and manifest artifacts together without changing the semantics of the existing demo-only or preview-only commands.
