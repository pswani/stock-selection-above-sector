...
- The first downstream normalization consumer is the factor layer: `normalize_factor_observations(...)` consumes `FactorObservation` records and returns a deterministic normalized factor frame, and `factors.normalize_factor_output(...)` exposes that handoff without introducing pillar logic.
