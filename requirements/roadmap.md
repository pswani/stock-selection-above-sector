...
1. Complete Milestone 3 provider contract expansion (corporate actions and ownership/short-interest now include additional safe alias mappings for split ratios and percentage ownership/short-interest fields; remaining work is any final safe field-coverage tightening plus explicit unsupported handling for gaps).
...
- Local validation on 2026-03-16 confirmed the bootstrap/validation scripts now run successfully; repo-wide Ruff baseline is now clean. Current readiness blocker is 5 pre-existing repo-wide Ruff UP042 findings outside active Milestone 3 scope; `uv sync --dev`, `uv run pytest -q`, and `uv run pyright` now pass locally.
