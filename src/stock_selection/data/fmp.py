...
class FinancialModelingPrepProvider:
    ...
    def get_corporate_actions(
        ...
        for ticker in tickers:
            try:
                dividend_payload = self._get_json(
            ...
            historical_dividends = (
                dividend_payload.get("historical", []) if isinstance(dividend_payload, dict) else []
            )
            for row in historical_dividends:
                if not row.get("date"):
                    continue
                snapshots.append(
                    CorporateActionSnapshot(
                        ...
                        value=self._as_float(row.get("dividend")),
                    )
                )
            ...
            historical_splits = split_payload.get("historical", []) if isinstance(split_payload, dict) else []
            for row in historical_splits:
                if not row.get("date"):
                    continue

                split_ratio = self._as_float(row.get("splitRatio"))
                if split_ratio is None:
                    numerator = self._as_float(row.get("numerator"))
                    denominator = self._as_float(row.get("denominator"))
                    split_ratio = numerator / denominator if numerator is not None and denominator else None
        ...
        if not endpoint_reachable:
            raise FmpProviderUnsupportedCapabilityError(
                "FMP corporate actions endpoint is not available for the current plan/API access."
            )

        snapshots.sort(key=lambda item: (item.ticker, item.as_of, item.action_type, item.value or 0.0))
        return snapshots
    ...
    def get_ownership_and_short_interest(
        ...
        for ticker in tickers:
            ...
            try:
                key_metrics = self._get_json(f"key-metrics-ttm/{ticker}")
                endpoint_reachable = True
            except FmpProviderError:
                key_metrics = []

            metric_row = key_metrics[0] if isinstance(key_metrics, list) and key_metrics else {}
            institutional_ownership = self._as_float(
                metric_row.get("institutionalOwnershipPercentageTTM")
                or metric_row.get("institutionalOwnershipPercentage")
            )
            insider_ownership = self._as_float(
                metric_row.get("insiderOwnershipPercentageTTM") or metric_row.get("insiderOwnershipPercentage")
            )

            try:
                short_interest = self._get_json(f"short-interest/{ticker}", query={"limit": 1})
                endpoint_reachable = True
            except FmpProviderError:
                short_interest = []

            short_row = short_interest[0] if isinstance(short_interest, list) and short_interest else {}
            short_interest_percent_float = self._as_float(
                short_row.get("shortPercentFloat") or short_row.get("shortInterestPercentFloat")
            )
            ...
            snapshots.append(
                OwnershipSnapshot(
                    ...
                    short_interest_percent_float=short_interest_percent_float,
                )
            )

        if not endpoint_reachable:
            raise FmpProviderUnsupportedCapabilityError(
                "FMP ownership/short-interest endpoint is not available for the current plan/API access."
            )
