...
def test_normalize_factor_output_orients_values_by_factor_direction() -> None:
    ...
    forward_pe = result[result["factor_name"] == "forward_pe"].reset_index(drop=True)
    ...
    roe = result[result["factor_name"] == "return_on_equity"].reset_index(drop=True)
...
def test_normalize_factor_output_keeps_missing_data_explicit() -> None:
    ...
    missing_group = result[result["ticker"] == "CCC"].iloc[0]
    ...
    valid_row = result[result["ticker"] == "AAA"].iloc[0]
    missing_value = result[result["ticker"] == "BBB"].iloc[0]
